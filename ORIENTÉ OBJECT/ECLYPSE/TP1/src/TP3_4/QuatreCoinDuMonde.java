...
public class QuatreCoinDuMonde {
	...
	public void seDeplacer(Chevalier c) {
	    ...
	    while (c.getPv() > 0 && c.getSac().size() < listeGraals.length && i< 10000000) {
	        ...
	        if (map[nouvelleX][nouvelleY] != null) {
	            ...
	            if (entiteSurCase instanceof Obstacles) {
	                ...
	                System.out.println("Obstacle touché ! : "+ entiteSurCase.getNom() +" / PV restants : " + c.getPv());
	            } 
	    ...
	    if (c.getPv() <= 0) {
	        ...
	        int mortX = c.getX();
	        int mortY = c.getY();
	        //jetter objtet apres mort
	        int indexObjet = 0;
	        for (int dx = -1; dx <= 1; dx++) {
	            for (int dy = -1; dy <= 1; dy++) {
	                
	                if (indexObjet < c.getSac().size() && !(dx == 0 && dy == 0)) {
	                    
	                    int voisinX = (mortX + dx + taille) % taille;
	                    int voisinY = (mortY + dy + taille) % taille;
	                    
	                    if (map[voisinX][voisinY] == null) {
	                        map[voisinX][voisinY] = c.getSac().get(indexObjet);
	                        indexObjet++;
	                    }
	                }
	            }
